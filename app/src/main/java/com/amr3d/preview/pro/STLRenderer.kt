...
class STLRenderer : GLSurfaceView.Renderer {
    ...
    private val shadowFragmentShaderCode = """
        ...
        void main() {
            float d = length(fUV);
            float alpha = smoothstep(1.0, 0.1, d) * 0.40;
    ...
    override fun onDrawFrame(unused: GL10?) {
        ...
        Matrix.translateM(modelMatrix, 0, -modelCenter[0], -modelCenter[1] + floatOffset, -modelCenter[2])
    ...
    /** بيرسم نسخة معكوسة شفافة جدًا من الموديل تحته — نفس فكرة .reflection في
     * amr3d_enhanced_v3.html. بيستخدم نفس بيانات الموديل وبرنامج التظليل، بس بمصفوفة
     * MVP معكوسة حول أرضية الموديل (نفس مستوى الظل) ودرجة شفافية منخفضة جدًا. */
    ...
    private fun drawGroundShadow() {
        ...
        // نطرح منها مركز الموديل لأن باقي السلسلة (T(-center)) بتفترض إحداثيات
        // نسبية للمركز، مش المطلقة من الملف
        val floorY = modelBottomY - modelCenter[1]
        val ext = r * 1.7f
